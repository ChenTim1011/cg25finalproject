    ...
    for (unsigned i = 0; i < number_of_impulses; ++i)
    {
        ...
        if (((x_i_x * x_i_x) + (y_i_y * y_i_y)) < 1.0)
        {
            if (isotropic_)
            {
                ...
                noise_v += w_i * gabor(K_, a_, F_0_, omega_0_i, x_i_x * kernel_radius_, y_i_y * kernel_radius_);
                noise_f += w_i * w_i * gabor_fre(K_, a_, F_0_, omega_0_i, (int_x + x) * kernel_radius_ / 256, (int_y + y) * kernel_radius_ / 256);
            }
            else
            {
                noise_v += w_i * gabor(K_, a_, F_0_, omega_0_, x_i_x * kernel_radius_, y_i_y * kernel_radius_);
